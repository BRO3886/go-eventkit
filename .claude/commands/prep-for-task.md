Bootstrap context for a new session on the `go-eventkit` project. Run this BEFORE starting any implementation work.
...
- All Go files in each package (`calendar/`, `reminders/`, root)
- All ObjC bridge files (`bridge_darwin.m`, `bridge_darwin.h`)
- Test files and integration scripts in `scripts/`
...
```
## Project State
- Phase 1 (Calendar): <status>
- Phase 2 (Reminders): <status>
- Shared types: <status>
- Coverage: calendar <X>%, reminders <X>%
